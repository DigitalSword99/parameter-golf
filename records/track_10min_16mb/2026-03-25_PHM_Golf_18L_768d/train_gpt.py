...
# HYPERPARAMETERS
# -----------------------------
# PHM-Golf: 18-layer / 768-dim Kronecker-factored transformer
# - PHMLinear replaces all standard linear layers (4x parameter reduction)
# - BigramHash augments token embeddings
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for dec_i in range(self.num_decoder_layers):
            eff_idx = self.num_encoder_layers + dec_i
            connections = self.skip_map.get(dec_i, [])
            for enc_i in connections:
